...
if __name__ == '__main__':
    ...
    trj_path = './'
    trj_file = 'slab_1k.lammpstrj'
    step     = 1
    scheme   = 'h3o'
    system   = 'sphere' # bulk slab
    ion_num  = 10
    ...
    if scheme == 'h3o':
        cutoff   = 1.25
        indexfile = open(os.path.join(trj_path, "{0}_ohidx_dump{1}_cut{2}_{3}.txt".format(scheme,step,cutoff,trj_name)), 'w+')
        h_xyzfile = open(os.path.join(trj_path, "{0}_h_xyz_dump{1}_cut{2}_{3}.txt".format(scheme,step,cutoff,trj_name)), 'w+')
        o_xyzfile = open(os.path.join(trj_path, "{0}_o_xyz_dump{1}_cut{2}_{3}.txt".format(scheme,step,cutoff,trj_name)), 'w+')
    ...
    for i in range(0,len(xyz_index),step):
